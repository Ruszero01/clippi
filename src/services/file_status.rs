...
/// Return a cached existence result without touching the source path on the
/// calling thread. A missing or expired entry schedules a bounded background
/// probe and returns the previous value (or None while initially unknown).
pub fn cached_file_exists(path: &str) -> Option<bool> {
    ...
    if cached.is_some_and(|entry| now.duration_since(entry.checked_at) < STATUS_TTL) {
        return cached.map(|entry| entry.exists);
    }
