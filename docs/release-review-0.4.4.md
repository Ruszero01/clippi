...
8. **发布配置。** 包及 bundle 版本统一为 0.4.4，补充更新记录。移除删除 release 并清理标签的流程，重试时更新现有 release，创建时要求标签已经存在。三平台格式、严格 lint、单元测试仅由版本标签触发的发布流程调用，普通分支推送及 PR 不触发 CI；发布计划依赖验证成功，并串行化同一 ref 的发布任务。Runner 标签参考 [GitHub 官方列表](https://docs.github.com/en/actions/reference/runners/github-hosted-runners)。
